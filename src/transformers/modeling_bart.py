...
BART_PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    "bart-large-mnli":
    "https://s3.amazonaws.com/models.huggingface.co/bert/facebook/bart-large-mnli/pytorch_model.bin",
    "bart-large-cnn":
    "https://s3.amazonaws.com/models.huggingface.co/bert/facebook/bart-large-cnn/pytorch_model.bin",
    "bart-large-xsum":
    "https://s3.amazonaws.com/models.huggingface.co/bert/facebook/bart-large-xsum/pytorch_model.bin",
}
...
def _prepare_bart_decoder_inputs(config,
    ...
    causal_mask = torch.triu(
        fill_with_neg_inf(torch.zeros(tgt_len, tgt_len)), 1).to(
...
class BartDecoder(nn.Module):
    ...
    def forward(self,
        ...
        positions = self.embed_positions(
            input_ids, generation_mode=generation_mode)

        if generation_mode:
            input_ids = input_ids[:, -1:]
            positions = positions[:, -1:]  # happens after we embed them
            # Dhruvesh
            # I do not know the purpose of this assert.
            # Removing as it get triggered when the decoder generates pad tokens
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            layer_state = decoder_cached_states[
                i] if decoder_cached_states is not None else None
...
class BartModel(PretrainedBartModel):
    ...
    def forward(
        ...
        decoder_outputs = _filter_out_falsey_values(
            decoder_outputs)  # type: tuple
        assert isinstance(decoder_outputs[0], torch.Tensor)
        encoder_outputs = _filter_out_falsey_values(
            encoder_outputs)  # type: tuple
    ...
    Examples::
        ...
        if lm_labels is not None:
            ...
            weight = torch.ones_like(
                lm_logits[0, 0])  # shape of logits=(batch, seq, vocab)
